...
colnames(merge_v3_small) <- c("cause of stroke","sex","race","age","hypertension","diabetes","hypercholestrolemia","pc1","pc2","pc3","pc4")
cor <-hetcor(merge_v3_small, ML=FALSE)
```
...
meltrcm=melt(rcm)
G=ggplot(meltrcm)
x <- G+geom_tile(aes(Var2,Var1,fill=value))+geom_text(aes(Var2,Var1,label=value))
...
```{r}
#use this webaite "http://www.sthda.com/english/wiki/ggplot2-quick-correlation-matrix-heatmap-r-software-and-data-visualization"
reorder_cormat <- function(cormat){
# Use correlation between variables as distance
dd <- as.dist((1-cormat)/2)
hc <- hclust(dd)
