...
# Matrix eQTL should include the interaction of SNP and the last covariate in the model and test for
# its significance
...
snpspos = read.table(snps_location_file_name, header = TRUE, stringsAsFactors = FALSE) #dim 628677 3, there is any info in annotation file for these two AFFX-SNP-000541 and AFFX-SNP-002255, so removed them 
...
```{r}
#save data
save(snps,gene,cvrt, file = "../../Input_Output/snps_gene_cvrt_final.Rdata") #These versions are final versions of input files (AFTER FILTERING AND NORMALIZATION)
...
noFDRsaveMemory = TRUE); #logical. Set noFDRsaveMemory = TRUE to save significant gene-SNP pairs directly to the output files, reduce memory footprint and skip FDR calculation. The eQTLs are not recorded in the returned object if noFDRsaveMemory = TRUE.
