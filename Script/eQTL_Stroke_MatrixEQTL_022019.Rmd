...
cisDist = 1e6 #keep this as a cis out of this distance conder as a trans
...
snpspos = read.table(snps_location_file_name, header = TRUE, stringsAsFactors = FALSE) #dim 628677 3, there is any info in annotation file for these two AFFX-SNP-000541 and AFFX-SNP-002255 in Axiom_BioBank1.na35.annot_v2 files, so removed them 
...
#now the dim snps 294089    351 the number of outliers are close to what smartpca and plink removed
```
...
#Error in saver.cis$getResults(gene, snps, n.tests.cis) : unused arguments (gene, snps, n.tests.cis) #becuase of CRAN version of this library
```
...
```{r}
## Results for MAF > 0.05, CR >99% (No significant associations were found.):

cat('Analysis done in: ', eQTL_try1$time.in.sec, ' seconds', '\n') #Analysis done in:  1087.04  seconds 
cat('Detected local eQTLs:', '\n');
show(eQTL_try1$cis$eqtls)
cat('Detected distant eQTLs:', '\n');
show(eQTL_try1$trans$eqtls)
#       snps            gene statistic       pvalue       FDR       beta
# 1 AX-11262710 TC14002163.hg.1  6.965680 1.676527e-11 0.2899372  0.3235115
# 2 AX-40778059 TC20001020.hg.1 -6.833977 3.783782e-11 0.3271821 -0.4521873
## Plot the Q-Q plot of local and distant p-values

plot(me)
```
```{r}
## Results for MAF > 0.01, CR >99% (No significant associations were found.):

cat('Analysis done in: ', eQTL_try2$time.in.sec, ' seconds', '\n') #Analysis done in:  1247.23  seconds 
cat('Detected local eQTLs:', '\n');
show(eQTL_try2$cis$eqtls)
cat('Detected distant eQTLs:', '\n');
show(eQTL_try2$trans$eqtls)
#         snps            gene statistic       pvalue       FDR       beta
# 1 AX-11262710 TC14002163.hg.1  6.965680 1.676527e-11 0.3794566  0.3235115
# 2 AX-40778059 TC20001020.hg.1 -6.833977 3.783782e-11 0.4282011 -0.4521873

#result eQTL_trt3 for MAF > 0.01, CR >95% (No significant associations were found.):
#       snps            gene statistic       pvalue       FDR       beta
# 1 AX-11262710 TC14002163.hg.1  6.965680 1.676527e-11 0.3794566  0.3235115
# 2 AX-40778059 TC20001020.hg.1 -6.833977 3.783782e-11 0.4282011 -0.4521873
```
